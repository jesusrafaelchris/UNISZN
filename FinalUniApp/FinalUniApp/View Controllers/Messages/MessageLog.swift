...
class MessageLog: UICollectionViewController, UITextViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    lazy var titleViewButton: UIButton = {
        ...
        titleViewButton.setTitle(self.username, for: .normal)
        titleViewButton.titleLabel?.font = UIFont.boldSystemFont(ofSize: 20)
        titleViewButton.titleLabel?.adjustsFontSizeToFitWidth = true
        titleViewButton.titleLabel?.minimumScaleFactor = 0.5

        // Create action listener
        titleViewButton.addTarget(self, action: #selector(titleViewButtonDidTap), for: .touchUpInside)
    ...
    lazy var inputContainerView: InputAccessoryView = {
        let chatInputContainerView = InputAccessoryView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 50))
    ...
    private func setUpCell(cell: ChatMessageCell, message: messageInfo) {
        ...
        if message.FromID == Auth.auth().currentUser?.uid{
            //outgoing message
            cell.bubbleView.backgroundColor = UIColor(red: 0.09, green: 0.64, blue: 0.68, alpha: 1.00)
            ...
            // incoming message
            cell.bubbleView.backgroundColor = UIColor(red: 0.21, green: 0.46, blue: 0.53, alpha: 1.00)
            cell.textView.textColor = .white
