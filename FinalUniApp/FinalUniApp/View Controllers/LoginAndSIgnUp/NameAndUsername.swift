...
func setupView() {
    ...
    checkIfUsernameTaken(username: userame) { (result) in
        ...
        InstanceID.instanceID().instanceID { (result, error) in
          if let error = error {
            ...
            guard let Surname = self.SurnameField.text else {return}
            guard let Username = self.UsernameField.text else {return}
            
            self.getProfilePictures { (ProfilePicture, Picture1, Picture2, Picture3) in
                
            let data: [String: Any] = ["FirstName": Firstname, "Surname": Surname, "username": Username, "uid": userID,
            ...
            batch.setData(data, forDocument: self.docRef.document(userID), merge: true)

            batch.setData(["NotificationToken": result.token], forDocument: self.docRef.document(userID), merge: true)
    ...
    func getProfilePictures(completion: @escaping(_ Picture1: String,_ Picture1: String,_ Picture2: String,_ Picture3: String) -> Void) {
        let ref = Firestore.firestore().collection("ProfilePictures").document("Pictures")
        ref.getDocument { (snapshot, error) in
            guard let data = snapshot?.data() else {return}
            guard let ProfilePicture = data["ProfilePicture"] as? String else {return}
            guard let Picture1 = data["Picture1"] as? String else {return}
            guard let Picture2 = data["Picture2"] as? String else {return}
            guard let Picture3 = data["Picture3"] as? String else {return}
            completion(ProfilePicture, Picture1, Picture2, Picture3)
        }
    }
